...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        # Define model
        # yaml.get(若找不到'ch'键值，返回ch) 故ch=3
        ch = self.yaml['ch'] = self.yaml.get('ch', ch)  # input channels
        # data.yaml中的nc不用修改，nc会被传进来的覆盖
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    """
    d为模型配置文件，例如yolov5s.yaml
    anchors:anchors
    nc:nc
    gd:depth_multiple网络深度
    gw:width_multiple网络宽度
    """
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    # na: number of anchors 每一个predict head上的anchor数 = 3
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    # no: number of outputs 每一个predict head层的输出channel = anchors * (classes + 5) = 75(VOC(voc20类)
    ...
    # from(当前层输入来自哪些层), number(当前层次数 初定), module(当前层类别), args(当前层类参数)
